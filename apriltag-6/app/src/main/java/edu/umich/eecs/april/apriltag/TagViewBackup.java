 ...
 */
public class TagViewBackup extends GLSurfaceView implements Camera.PreviewCallback {
    ...
    public TagViewBackup(Context context, SurfaceHolder overlay) {
        super(context);
        ...
        mRenderer = new TagViewBackup.Renderer();
    ...
    class Renderer implements GLSurfaceView.Renderer {
        ...
        public double getAngle (double x1,double y1,double x2, double y2, double x3, double y3)
        {
            ...
            double g1 = anyTwoPoint(x1-x2, y1-y2);

            double g2 = anyTwoPoint(x3 - x2, y3 - y2);
            double del = Math.abs(g1-g2);


            if (del > Math.PI) del = 2 * Math.PI - del;

            return Math.toDegrees(del);


        }

        private double anyTwoPoint(double a, double b) {
        ...
        public void onDrawFrame(GL10 gl) {
            ...
            //mGameHandler.sendMessage(Message.obtain(mGameHandler, 2));
            ...
            if ((mDetections != null )
                ...
                /*
                    for (ApriltagDetection det : mDetections) {
                        ...
                        for (int j = 0; j < 4; j += 1) {
                            double x = 0.5 - (det.p[2 * j + 1] / mPreviewSize.height);
                            double y = 0.5 - (det.p[2 * j + 0] / mPreviewSize.width);
                            points[2 * j + 0] = (float) x;
                            points[2 * j + 1] = (float) y;
                        }
                ...
                */
                ...
                for (ApriltagDetection det : mDetections) {
                    ...
                    //System.out.println(mDetections.size());
                    ...
                    double cx = (det.c[1]);
                    double cy =  (det.c[0]);
                    center[2 * m + 0] = (float) cx;
                    center[2 * m + 1] = (float) cy;

                    cords[2 * m + 0] = (float)det.c[1];
                    cords[2 * m + 1] = (float) det.c[0];
                ...
                if (mDetections != null) {
                    if (center[5] != 0.00)
                    {
                        ...
                        float[] point_0c = Arrays.copyOfRange(center, 0, 2);
                        float[] point_1c = Arrays.copyOfRange(center, 2, 4);
                        float[] point_2c = Arrays.copyOfRange(center, 4, 6);
                        ...
                        // System.out.println("x1 "+x1+" y1 "+y1+" x2 "+x2+" y2 "+y2+" x3 "+x3+" y3 "+y2);
                        ...
                        double alfa = getAngle(x3, y3, x1, y1, x2, y2);




                        double gamaAli = getAngle(x1Ali, y1Ali, x3Ali, y3Ali, x2Ali, y2Ali);
                        double betaAli = getAngle(x1Ali, y1Ali, x2Ali, y2Ali, x3Ali, y3Ali);
                        double alfaAli = getAngle(x2Ali, y2Ali, x1Ali, y1Ali, x3Ali, y3Ali);



                        DecimalFormat df = new DecimalFormat("#.##");
                        System.out.println("alpha " + df.format(alfa)+" Beta  "+df.format(gama)+"  Gama "+ df.format(beta) );


                        //System.out.println("alpha Ali " + alfaAli+" "+gamaAli+" "+ betaAli );
                        /*
                        double atanSlopeA = ((Math.atan((y2-y1)/(x2-x1)))  -   (Math.atan((y3-y1)/(x3-x1))) );
                        double atanSlopeB = (((Math.atan((y1-y2)/(x1-x2)))  -   (Math.atan((y3-y2)/(x3-x2))) ));
                        double atanSlopeC = ((Math.atan((y1-y3)/(x1-x3)))  -   (Math.atan((y2-y3)/(x2-x3))) );





                        System.out.println(atanSlopeA+" "+atanSlopeB+" "+atanSlopeC+" "+"tiissss");





                        double angleA = Math.toDegrees(atanSlopeA);

                        double angleB = Math.toDegrees(atanSlopeB);

                        double angleC = Math.toDegrees(atanSlopeC);

                        double [] angles = {angleA,angleB,angleC};

                        System.out.println((atanSlopeA -  atanSlopeB)+" "+(atanSlopeA -  atanSlopeC)+" "+(atanSlopeB -  atanSlopeC)+" "+"for aliii");


                        int count = 0;
                        double y = 0.00;
                        //System.out.println("angles before check:"+ angleA+" "+ angleB+" "+ angleC );
                        for(double x : angles) {
                        ...
                        //mGameHandler.sendMessage(Message.obtain(mGameHandler,0,angles));
...
/*
/*
                        double l0 = (Math.sqrt(Math.pow((x1-x0),2))+Math.pow((y1-y0),2));

                        double l1=  (Math.sqrt(Math.pow((x2-x1),2))+Math.pow((y2-y1),2));

                        double l2 = (Math.sqrt(Math.pow((x2-x0),2))+Math.pow((y2-y0),2));


                        double cosC= ((Math.pow(l0,2)+Math.pow(l1,2)-Math.pow(l2,2)) /(2*l0*l1));

                        double cosB = ((Math.pow(l2,2)+Math.pow(l0,2)-Math.pow(l1,2)) /(2*l2*l0));

                        double cosA = ((Math.pow(l1,2)+Math.pow(l2,2)-Math.pow(l0,2)) /(2*l1*l2));
                        ...
                        double angleBB=Math.toDegrees(acosB);

                        double angleCC=Math.toDegrees(acosC);

                        double angleAA=Math.toDegrees(acosA);
                        ...
                        System.out.println("daraje A " +angleAA +" B "+angleBB +"  "+angleCC);


                       // System.out.println("zavie A "+angleA);
...
*/
                ...
                // float[] line_yc = new float[]{point_0c[0], point_0c[1], point_3c[0], point_3[c1]};



                // float[] line_borderc = new float[]{point_1c[0], point_1c[1], point_2c[0], point_2c[1],
                //        point_2c[0], point_2c[1], point_3c[0], point_3c[1]};

                // Draw lines
                // lp.draw(PVM, line_xc, 2, COLOR_GREEN, GLES20.GL_LINES);
                //lp.draw(PVM, line_yc, 2, COLOR_RED, GLES20.GL_LINES);
                // lp.draw(PVM, line_borderc, 4, COLOR_BLUE, GLES20.GL_LINES);
